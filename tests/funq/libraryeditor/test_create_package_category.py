...
def test(library_editor, helpers):
    ...
    # Check metadata
    assert (
        le.widget("libraryEditorPkgCatNameEdit").properties()["text"]
        == "New Package Category"
    )
    assert (
        le.widget("libraryEditorPkgCatDescriptionEdit").properties()["plainText"]
        == "Foo Bar"
    )
    assert le.widget("libraryEditorPkgCatKeywordsEdit").properties()["text"] == ""
    assert (
        le.widget("libraryEditorPkgCatAuthorEdit").properties()["text"]
        == "Functional Test"
    )
