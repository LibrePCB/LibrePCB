...
"""
Test image export dialog in footprint editor

Meant as an addition to the tests in test_schematic_image_export.py just to
test the footprint editor specific functionality works as well.
"""
...
def dialog(library_editor, helpers):
    """
    Fixture opening the image export dialog in the footprint editor
    """
    list_widget = library_editor.widget('libraryEditorOverviewPkgList')
    helpers.wait_for_model_items_count(list_widget, 1)
    item = list_widget.model().items().items[0]
    list_widget.dclick_item(item)

    library_editor.action('libraryEditorActionExportImage').\
