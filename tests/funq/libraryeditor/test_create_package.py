...
def test(library_editor, helpers):
    ...
    # Check metadata
    assert (
        le.widget("libraryEditorPackageNameEdit").properties()["text"] == "New Package"
    )
    assert (
        le.widget("libraryEditorPackageDescriptionEdit").properties()["plainText"]
        == "Foo Bar"
    )
    assert le.widget("libraryEditorPackageKeywordsEdit").properties()["text"] == ""
    assert (
        le.widget("libraryEditorPackageAuthorEdit").properties()["text"]
        == "Functional Test"
    )
