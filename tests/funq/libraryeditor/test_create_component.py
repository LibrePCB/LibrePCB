...
def test(library_editor, helpers):
    ...
    for (widget, property), value in widget_properties.items():
        le.widget(
            "libraryEditorNewElementWizardComponentProperties" + widget
    ...
    category_tree = le.widget(
        "libraryEditorNewElementWizardComponentSymbolsChooseSymbolDialogCategoriesTree"
    )
    ...
    symbols_list = le.widget(
        "libraryEditorNewElementWizardComponentSymbolsChooseSymbolDialogSymbolsList"
    )
    helpers.wait_for_model_items_count(symbols_list, 1)
    symbol = symbols_list.model().items().items[0]
    symbols_list.select_item(symbol)
    le.widget(
        "libraryEditorNewElementWizardComponentSymbolsChooseSymbolDialogAcceptButton"
    ).click()
    ...
    le.widget(
        "libraryEditorNewElementWizardComponentPinSignalMapAutoAssignButton"
    ).click()
    ...
    # Check metadata
    assert (
        le.widget("libraryEditorComponentNameEdit").properties()["text"]
        == "New Component"
    )
    assert (
        le.widget("libraryEditorComponentDescriptionEdit").properties()["plainText"]
        == "Foo Bar"
    )
    assert le.widget("libraryEditorComponentKeywordsEdit").properties()["text"] == ""
    assert (
        le.widget("libraryEditorComponentAuthorEdit").properties()["text"]
        == "Functional Test"
    )
    assert (
        le.widget("libraryEditorComponentVersionEdit").properties()["text"] == "1.2.3"
    )
