...
def test_metadata(library_editor):
    ...
    assert (
        le.widget("libraryEditorOverviewNameEdit").properties()["text"]
        == "Populated Library"
    )
    assert (
        le.widget("libraryEditorOverviewDescriptionEdit").properties()["plainText"]
        == "Test library containing some elements"
    )
    assert (
        le.widget("libraryEditorOverviewKeywordsEdit").properties()["text"]
        == "test,library,populated"
    )
