...
protected:
  FilePath mTmpDir;
  static constexpr const char* sUuid = "35aad2af-5cd4-42ae-8576-fe7febad0d8a";
...
TEST_F(SymbolTest, testUpgradeV01) {
  // Copy into temporary directory.
  const FilePath src =
      FilePath(TEST_DATA_DIR "/libraries/v0.1.lplib/sym").getPathTo(sUuid);
  FileUtils::copyDirRecursively(src, mTmpDir);

  // Open/upgrade/close.
  ASSERT_TRUE(FileUtils::readFile(mTmpDir.getPathTo(".librepcb-sym"))
                  .startsWith("0.1\n"));
  {
    std::unique_ptr<Symbol> obj = Symbol::open(createDir());
    obj->save();
  ...
  // Re-open.
  ASSERT_TRUE(
      FileUtils::readFile(mTmpDir.getPathTo(".librepcb-sym"))
