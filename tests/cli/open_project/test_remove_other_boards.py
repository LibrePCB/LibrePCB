...
def test_project_with_two_boards_remove_both_volatile(cli, project):
    ...
    for _ in range(2):
        code, stdout, stderr = cli.run(
            "open-project", "--remove-other-boards", project.path
        )
        assert stderr == ""
...
def test_project_with_two_boards_remove_one_and_save(cli, project):
    ...
    code, stdout, stderr = cli.run(
        "open-project", "--board=copy", "--remove-other-boards", "--save", project.path
    )
    assert stderr == ""
    assert stdout == nofmt(f"""\
Open project '{project.path}'...
...
""")
    ...
    code, stdout, stderr = cli.run(
        "open-project", "--board=copy", "--remove-other-boards", project.path
    )
    assert stderr == ""
    assert stdout == nofmt(f"""\
Open project '{project.path}'...
