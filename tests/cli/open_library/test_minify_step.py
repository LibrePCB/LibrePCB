...
def test_save(cli):
    ...
    code, stdout, stderr = cli.run(
        "open-library", "--all", "--minify-step", "--save", library.dir
    )
...
Process {library.corp} corporates...
SUCCESS
""")
    assert code == 0
...
def test_strict(cli):
    ...
    code, stdout, stderr = cli.run(
        "open-library", "--all", "--minify-step", "--strict", library.dir
    )
    if "LibrePCB was compiled without OpenCascade" in stderr:
        pytest.skip("Feature not available.")
...
def test_invalid_file(cli):
    ...
    with open(path, "wb") as f:
        f.write(b"invalid")
    code, stdout, stderr = cli.run(
        "open-library", "--all", "--minify-step", library.dir
    )
    if "LibrePCB was compiled without OpenCascade" in stderr:
        pytest.skip("Feature not available.")
    assert stderr == nofmt(f"""\
