...
ProjectLibraryUpdater::ProjectLibraryUpdater(workspace::Workspace& ws, const FilePath& project,
                                             ControlPanel& cp) noexcept:
    QMainWindow(nullptr), mWorkspace(ws), mProjectFilePath(project), mControlPanel(cp),
    mUi(new Ui::ProjectLibraryUpdater)
{
    mUi->setupUi(this);
    mUi->btnUpdate->setText(mUi->btnUpdate->text().arg(mProjectFilePath.getBasename()));
    connect(mUi->btnUpdate, &QPushButton::clicked, this, &ProjectLibraryUpdater::btnUpdateClicked);
}
...
void ProjectLibraryUpdater::btnUpdateClicked()
{
    ...
    bool abort = false;
    project::editor::ProjectEditor* editor = mControlPanel.getOpenProject(mProjectFilePath);
    if (editor) {
        log(tr("Ask to close project (confirm message box!)"));
        bool close = editor->closeAndDestroy(true, this);
        if (close) {
            delete editor; // delete editor to make sure the lock is released immediately
        } else {
            abort = true;
            log(tr("Abort."));
        }
    ...
    if (!abort) {
        try {
            ...
            FilePath libDirBackup = mProjectFilePath.getParentDir().getPathTo("library~");
            ...
            try {
                ...
                throw RuntimeError(__FILE__, __LINE__,
                    tr("Failed to update library elements! Probably there were breaking "
        ...
        if (editor) {
            ...
            // bring this window to front again (with some delay to make it working properly)
...
void ProjectLibraryUpdater::updateElements(const QString& type,
