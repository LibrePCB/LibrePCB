...
4. Update the installer version in `ci/build_installer.sh` (e.g. "0.1.0-1"),
   commit. Note: Always append a dash followed by a number!
5. Update the releases section in
...
7. Publish the artifacts at https://download.librepcb.org/releases/.
8. Test the official releases again, this time including the installer.
9. Make sure all submodules point to persistent commits (commits could be
...
10. If everything works properly, add and push a new tag with the name equal to
   ...
   instead of "0.1")!
11. Bump application and installer version on `master`.
12. Update website/documentation download links and publish blog post
    containing the changelog.
13. Add tag description to [GitHub Releases], including a link to the blog post.
14. Notify users through Patreon, Twitter, ...
15. Start updating distribution packages (Flatpak, Arch, NixOS, ...).
