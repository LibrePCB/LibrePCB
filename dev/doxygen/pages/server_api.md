...
| Path         | Description                           |
|--------------|---------------------------------------|
| [/info]      | Get information about the API server  |
...
|-----------|----------------|------------------------------------------------|
| libraries | `null` or `{}` | Whether the `/libraries` resource is supported |
| order     | `null` or `{}` | Whether the `/order` resource is supported     |
| parts     | `null` or `{}` | Whether the `/parts` resource is supported     |

Typically those childs are empty objects but may be extended with properties
in future. Unsupported resources may either not appear in the response at all
(preferred), or as `null` values.
