...
void LibraryManager::clearLibraryList() noexcept
{
    for (int i = mUi->lstLibraries->count()-1; i >= 0; i--) {
        QListWidgetItem* item = mUi->lstLibraries->item(i); Q_ASSERT(item);
...
void LibraryManager::loadLibraryList() noexcept
{
    ...
    widgets.append(new LibraryListWidgetItem(mWorkspace, QSharedPointer<library::Library>()));
...
void LibraryManager::currentListItemChanged(QListWidgetItem* current, QListWidgetItem* previous) noexcept
{
    ...
    if (current) {
        LibraryListWidgetItem* item = dynamic_cast<LibraryListWidgetItem*>(mUi->lstLibraries->itemWidget(current));
...
void LibraryManager::libraryAddedSlot(const FilePath& libDir, bool select) noexcept
{
    clearLibraryList();
    loadLibraryList();
    ...
    if (select) {
        ...
        for (int i = 0; i < mUi->lstLibraries->count(); i++) {
            QListWidgetItem* item = mUi->lstLibraries->item(i); Q_ASSERT(item);
            LibraryListWidgetItem* widget = dynamic_cast<LibraryListWidgetItem*>(mUi->lstLibraries->itemWidget(item));
            if (widget && widget->getLibrary() && widget->getLibrary()->getFilePath() == libDir) {
