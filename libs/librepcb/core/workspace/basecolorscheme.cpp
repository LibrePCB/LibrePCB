...
ColorScheme::Colors ColorScheme::getColors(const QString& role) const noexcept {
  ...
  qCritical().nospace() << "Color scheme " << getName()
...
const BaseColorScheme& BaseColorScheme::view3dLibrePcbDefault() noexcept {
  static const BaseColorScheme instance = create(  //
      "b3c5c2ed-45bc-47f5-b639-3713c6db826d",  // UUID
      "LibrePCB Default",  // Name
      {
          // Use a background color which ensures good contrast to both black
          // and white STEP models. The secondary color is used e.g. for overlay
          // buttons.
