...
void WorkspaceLibraryDbWriter::createAllTables() {
  ...
  queries << QString(
      ...
      ")");
  queries << QString(
      "CREATE TABLE IF NOT EXISTS corporate_pcb_products ("
      "`id` INTEGER PRIMARY KEY NOT NULL, "
      ...
      ")");
  queries << QString(
      "CREATE TABLE IF NOT EXISTS corporate_output_jobs ("
      "`id` INTEGER PRIMARY KEY NOT NULL, "
      "`corporate_id` INTEGER REFERENCES corporates(id) "
      "ON DELETE CASCADE NOT NULL, "
...
int WorkspaceLibraryDbWriter::addCorporatePcbProduct(
    int corpId, const Uuid& uuid, const QString& name,
    const QString& description, const QUrl& url, int maxLayers) {
  QSqlQuery query = mDb.prepareQuery(
      "INSERT INTO corporate_pcb_products "
      "(corporate_id, uuid, name, description, url, max_layers) VALUES "
      "(:corporate_id, :uuid, :name, :description, :url, :max_layers)");
...
int WorkspaceLibraryDbWriter::addCorporateOutputJob(
    int corpId, WorkspaceLibraryDb::OutputJobKind kind, const Uuid& uuid,
    const QString& type, const QString& name) {
  QSqlQuery query = mDb.prepareQuery(
      "INSERT INTO corporate_output_jobs "
      "(corporate_id, kind, uuid, type, name) VALUES "
      "(:corporate_id, :kind, :uuid, :type, :name)");
  query.bindValue(":corporate_id", corpId);
