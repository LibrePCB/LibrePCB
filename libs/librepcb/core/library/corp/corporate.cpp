...
std::shared_ptr<const OutputJob> Corporate::findPcbOutputJob(
...
void Corporate::setPcbProducts(
    const QVector<CorporatePcbProduct>& products) noexcept {
  if (products != mPcbProducts) {
    mPcbProducts = products;
    emit pcbProductsModified();
  }
}
