...
template <>
inline Pad::Shape deserialize(const SExpression& node) {
...
template <>
inline Pad::ComponentSide deserialize(const SExpression& node) {
  const QString str = node.getValue();
...
template <>
inline Pad::Function deserialize(const SExpression& node) {
  const QString str = node.getValue();
