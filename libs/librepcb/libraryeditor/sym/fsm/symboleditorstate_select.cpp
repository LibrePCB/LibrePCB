        ...
        case SubState::IDLE: {
            ...
            if (count == 0) {
                ...
                if (dynamic_cast<SymbolPinGraphicsItem*>(topMostItem)) {
                    ...
                    dynamic_cast<SymbolPinGraphicsItem*>(topMostItem)->setSelected(true);
                } else {
...
bool SymbolEditorState_Select::processGraphicsSceneRightMouseButtonReleased(QGraphicsSceneMouseEvent& e) noexcept
{
    if (mState == SubState::IDLE) {
...
bool SymbolEditorState_Select::openPropertiesDialogOfItemAtPos(const Point& pos) noexcept
{
    ...
    if (pins.count() > 0) {
        ...
        SymbolPinPropertiesDialog dialog(item->getPin(), mContext.undoStack, &mContext.editorWidget);
        dialog.exec();
        return true;
    } else if (texts.count() > 0) {
