...
PackageEditorState_DrawPolygonBase::PackageEditorState_DrawPolygonBase(Context& context,
                                                                       Mode mode) noexcept :
    PackageEditorState(context), mMode(mode), mCurrentPolygon(nullptr),
    mCurrentGraphicsItem(nullptr), mLastLayerName(GraphicsLayer::sTopPlacement),
    mLastLineWidth(250000), mLastAngle(0), mLastFill(false),
    mLastGrabArea(mode != Mode::LINE)
{
}

PackageEditorState_DrawPolygonBase::~PackageEditorState_DrawPolygonBase() noexcept
{
...
bool PackageEditorState_DrawPolygonBase::entry() noexcept
{
    ...
    if (mMode != Mode::RECT) {
        ...
        connect(angleSpinBox.get(),
                static_cast<void(QDoubleSpinBox::*)(double)>(&QDoubleSpinBox::valueChanged),
                this, &PackageEditorState_DrawPolygonBase::angleSpinBoxValueChanged);
    ...
    if (mMode != Mode::LINE) {
        ...
        connect(fillCheckBox.get(), &QCheckBox::toggled,
                this, &PackageEditorState_DrawPolygonBase::fillCheckBoxCheckedChanged);
    ...
    if (mMode != Mode::LINE) {
        ...
        connect(grabAreaCheckBox.get(), &QCheckBox::toggled,
                this, &PackageEditorState_DrawPolygonBase::grabAreaCheckBoxCheckedChanged);
...
bool PackageEditorState_DrawPolygonBase::processGraphicsSceneLeftMouseButtonDoubleClicked(QGraphicsSceneMouseEvent& e) noexcept
{
...
bool PackageEditorState_DrawPolygonBase::start(const Point& pos) noexcept
{
    try {
        ...
        mCurrentGraphicsItem = mContext.currentGraphicsItem->getPolygonGraphicsItem(*mCurrentPolygon);
...
bool PackageEditorState_DrawPolygonBase::addNextSegment(const Point& pos) noexcept
{
    try {
        // commit current
...
bool PackageEditorState_DrawPolygonBase::updateCurrentPosition(const Point& pos) noexcept
{
...
void PackageEditorState_DrawPolygonBase::layerComboBoxValueChanged(const QString& layerName) noexcept
{
...
void PackageEditorState_DrawPolygonBase::lineWidthSpinBoxValueChanged(double value) noexcept
{
...
void PackageEditorState_DrawPolygonBase::angleSpinBoxValueChanged(double value) noexcept
{
...
void PackageEditorState_DrawPolygonBase::fillCheckBoxCheckedChanged(bool checked) noexcept
{
    mLastFill = checked;
    if (mEditCmd) {
...
void PackageEditorState_DrawPolygonBase::grabAreaCheckBoxCheckedChanged(bool checked) noexcept
{
