...
bool PackageEditorState_Select::processGraphicsSceneMouseMoved(QGraphicsSceneMouseEvent& e) noexcept
{
    ...
    switch (mState) {
        ...
        case SubState::MOVING: {
            if (!mCmdMoveSelectedItems) {
                mCmdMoveSelectedItems.reset(new CmdMoveSelectedFootprintItems(mContext, startPos));
            }
...
bool PackageEditorState_Select::processGraphicsSceneLeftMouseButtonPressed(QGraphicsSceneMouseEvent& e) noexcept
{
        ...
        case SubState::IDLE: {
            ...
            if (count == 0) {
                ...
                if (dynamic_cast<FootprintPadGraphicsItem*>(topMostItem)) {
                    ...
                    dynamic_cast<FootprintPadGraphicsItem*>(topMostItem)->setSelected(true);
                } else {
...
bool PackageEditorState_Select::processGraphicsSceneLeftMouseButtonReleased(QGraphicsSceneMouseEvent& e) noexcept
{
    Q_UNUSED(e);
    switch (mState) {
        case SubState::SELECTING: {
...
bool PackageEditorState_Select::processGraphicsSceneLeftMouseButtonDoubleClicked(QGraphicsSceneMouseEvent& e) noexcept
{
...
bool PackageEditorState_Select::processGraphicsSceneRightMouseButtonReleased(QGraphicsSceneMouseEvent& e) noexcept
{
    if (mState == SubState::IDLE) {
...
bool PackageEditorState_Select::openPropertiesDialogOfItemAtPos(const Point& pos) noexcept
{
    ...
    if (pads.count() > 0) {
        ...
        FootprintPadPropertiesDialog dialog(mContext.package, *mContext.currentFootprint,
                                            item->getPad(), mContext.undoStack,
        ...
        PolygonGraphicsItem* item = dynamic_cast<PolygonGraphicsItem*>(polygons.first().data()); Q_ASSERT(item);
        PolygonPropertiesDialog dialog(item->getPolygon(), mContext.undoStack,
        ...
        HoleGraphicsItem* item = dynamic_cast<HoleGraphicsItem*>(holes.first().data()); Q_ASSERT(item);
        HolePropertiesDialog dialog(item->getHole(), mContext.undoStack, &mContext.editorWidget);
        dialog.exec();
        return true;
    } else {
        return false;
...
bool PackageEditorState_Select::rotateSelectedItems(const Angle& angle) noexcept
{
    try {
        mContext.undoStack.execCmd(new CmdRotateSelectedFootprintItems(mContext, angle));
    } catch (const Exception& e) {
        QMessageBox::critical(&mContext.editorWidget, tr("Error"), e.getMsg());
    }
...
void PackageEditorState_Select::setSelectionRect(const Point& p1, const Point& p2) noexcept
{
    mContext.graphicsScene.setSelectionRect(p1, p2);
    mContext.currentGraphicsItem->setSelectionRect(QRectF(p1.toPxQPointF(), p2.toPxQPointF()));
}

void PackageEditorState_Select::clearSelectionRect(bool updateItemsSelectionState) noexcept
{
