...
LibraryOverviewWidget::LibraryOverviewWidget(const Context& context,
    ...
    EditorWidgetBase(context, lib->getFilePath(), parent), mLibrary(lib),
    mUi(new Ui::LibraryOverviewWidget)
{
    ...
    mDependenciesEditorWidget.reset(new LibraryListEditorWidget(mContext.workspace, this));
    int row;
    ...
    connect(mUi->btnIcon, &QPushButton::clicked, this, &LibraryOverviewWidget::btnIconClicked);
    connect(mUi->edtName, &QLineEdit::textChanged, this, &QWidget::setWindowTitle);
    connect(mUi->edtName, &QLineEdit::textEdited, this, &LibraryOverviewWidget::setDirty);
    connect(mUi->edtDescription, &QPlainTextEdit::textChanged, this, &LibraryOverviewWidget::setDirty);
    connect(mUi->edtKeywords, &QLineEdit::textEdited, this, &LibraryOverviewWidget::setDirty);
    connect(mUi->edtAuthor, &QLineEdit::textEdited, this, &LibraryOverviewWidget::setDirty);
    connect(mUi->edtVersion, &QLineEdit::textEdited, this, &LibraryOverviewWidget::setDirty);
    connect(mUi->edtUrl, &QLineEdit::textEdited, this, &LibraryOverviewWidget::setDirty);
    connect(mUi->cbxDeprecated, &QCheckBox::clicked, this, &LibraryOverviewWidget::setDirty);
    connect(mDependenciesEditorWidget.data(), &LibraryListEditorWidget::libraryAdded,
            this, &LibraryOverviewWidget::setDirty);
    connect(mDependenciesEditorWidget.data(), &LibraryListEditorWidget::libraryRemoved,
            this, &LibraryOverviewWidget::setDirty);

    updateElementLists();
    connect(&mContext.workspace.getLibraryDb(), &workspace::WorkspaceLibraryDb::scanSucceeded,
            this, &LibraryOverviewWidget::updateElementLists);
...
{
    mUi->btnIcon->setToolTip(tr("Click here to choose an icon (PNG, 256x256px)."));
...
{
    updateElementList<ComponentCategory>(*mUi->lstCmpCat, QIcon(":/img/places/folder.png"));
    updateElementList<PackageCategory>(  *mUi->lstPkgCat, QIcon(":/img/places/folder_green.png"));
    ...
    updateElementList<Component>(        *mUi->lstCmp,    QIcon(":/img/library/component.png"));
    updateElementList<Device>(           *mUi->lstDev,    QIcon(":/img/library/device.png"));
}

template <typename ElementType>
void LibraryOverviewWidget::updateElementList(QListWidget& listWidget, const QIcon& icon) noexcept
{
    QHash<FilePath, QString> elementNames;

    try {
        // get all library element names
        QList<FilePath> elements = mContext.workspace.getLibraryDb().getLibraryElements
                                   <ElementType>(mLibrary->getFilePath()); // can throw
    ...
    for (int i = listWidget.count() - 1; i >= 0; --i) {
        QListWidgetItem* item = listWidget.item(i); Q_ASSERT(item);
