...
EditorWidgetBase::EditorWidgetBase(const Context& context, const FilePath& fp,
                                   QWidget* parent) :
    QWidget(parent), mContext(context), mFilePath(fp), mUndoStackActionGroup(nullptr),
    mToolsActionGroup(nullptr), mIsDirty(false), mIsInterfaceBroken(false)
{
    mUndoStack.reset(new UndoStack());
    connect(mUndoStack.data(), &UndoStack::cleanChanged,
            this, &EditorWidgetBase::undoStackCleanChanged);
    connect(mUndoStack.data(), &UndoStack::stateModified,
            this, &EditorWidgetBase::undoStackStateModified);
...
void EditorWidgetBase::setUndoStackActionGroup(UndoStackActionGroup* group) noexcept
{
    ...
    if (mUndoStackActionGroup) mUndoStackActionGroup->setUndoStack(mUndoStack.data());
}

void EditorWidgetBase::setToolsActionGroup(ExclusiveActionGroup* group) noexcept
{
...
void EditorWidgetBase::setupInterfaceBrokenWarningWidget(QWidget& widget) noexcept
{
    ...
    label->setText(tr("WARNING: You have changed some important properties of this "
    ...
    connect(this, &EditorWidgetBase::interfaceBrokenChanged,
...
void EditorWidgetBase::toolActionGroupChangeTriggered(const QVariant& newTool) noexcept
{
