...
SymbolChooserDialog::SymbolChooserDialog(const workspace::Workspace& ws,
    ...
    mUi(new Ui::SymbolChooserDialog), mPreviewScene(new GraphicsScene())
{
    mUi->setupUi(this);
    ...
    connect(mUi->treeCategories->selectionModel(), &QItemSelectionModel::currentChanged,
            this, &SymbolChooserDialog::treeCategories_currentItemChanged);
    connect(mUi->listSymbols, &QListWidget::currentItemChanged,
            this, &SymbolChooserDialog::listSymbols_currentItemChanged);
    connect(mUi->listSymbols, &QListWidget::itemDoubleClicked,
            this, &SymbolChooserDialog::listSymbols_itemDoubleClicked);
...
{
    return mSelectedSymbol ? mSelectedSymbol->getDescriptions().value(localeOrder()) : QString();
}
...
void SymbolChooserDialog::treeCategories_currentItemChanged(const QModelIndex& current,
...
void SymbolChooserDialog::listSymbols_itemDoubleClicked(QListWidgetItem* item) noexcept
{
    if (item) {
...
{
    if (!mSelectedSymbol) {
        QMessageBox::information(this, tr("Invalid Selection"), tr("Please select a symbol."));
        return;
    }
    QDialog::accept();
}
