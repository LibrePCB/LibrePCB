...
ComponentEditorWidget::ComponentEditorWidget(const Context& context, const FilePath& fp,
                                             QWidget* parent) :
    EditorWidgetBase(context, fp, parent), mUi(new Ui::ComponentEditorWidget)
{
    ...
    connect(mUi->attributesEditorWidget, &AttributeListEditorWidget::edited,
            this, &ComponentEditorWidget::setDirty);
    ...
    connect(mUi->cbxSchematicOnly, &QCheckBox::toggled,
            this, &ComponentEditorWidget::undoStackStateModified);

    // set dirty state when properties are modified
    connect(mUi->edtName, &QLineEdit::textEdited, this, &ComponentEditorWidget::setDirty);
    connect(mUi->edtDescription, &QPlainTextEdit::textChanged, this, &ComponentEditorWidget::setDirty);
    connect(mUi->edtKeywords, &QLineEdit::textEdited, this, &ComponentEditorWidget::setDirty);
    connect(mUi->edtAuthor, &QLineEdit::textEdited, this, &ComponentEditorWidget::setDirty);
    connect(mUi->edtVersion, &QLineEdit::textEdited, this, &ComponentEditorWidget::setDirty);
    connect(mUi->cbxDeprecated, &QCheckBox::clicked, this, &ComponentEditorWidget::setDirty);
    ...
    connect(mCategoriesEditorWidget.data(), &ComponentCategoryListEditorWidget::categoryRemoved,
            this, &ComponentEditorWidget::setDirty);
    connect(mUi->cbxSchematicOnly, &QCheckBox::clicked, this, &ComponentEditorWidget::setDirty);
    connect(mUi->edtPrefix, &QLineEdit::textEdited, this, &ComponentEditorWidget::setDirty);
    connect(mUi->edtDefaultValue, &QPlainTextEdit::textChanged, this, &ComponentEditorWidget::setDirty);
}
...
bool ComponentEditorWidget::openComponentSymbolVariantEditor(ComponentSymbolVariant& variant) noexcept
{
...
{
    if (mUi->cbxSchematicOnly->isChecked() != mOriginalIsSchematicOnly)         return true;
    if (mComponent->getSignals().getUuidSet() != mOriginalSignalUuids)          return true;
    for (const ComponentSymbolVariant& original : mOriginalSymbolVariants) {
        const ComponentSymbolVariant* current = mComponent->getSymbolVariants().find(original.getUuid()).get();
        if (!current)                                                           return true;
        if (current->getSymbolItems().getUuidSet() != original.getSymbolItems().getUuidSet()) return true;
        for (const ComponentSymbolVariantItem& originalItem : original.getSymbolItems()) {
            const ComponentSymbolVariantItem* currentItem = current->getSymbolItems().find(originalItem.getUuid()).get();
            if (currentItem->getSymbolUuid() != originalItem.getSymbolUuid())   return true;
            if (currentItem->getPinSignalMap().getUuidSet() != originalItem.getPinSignalMap().getUuidSet()) return true;
            for (const ComponentPinSignalMapItem& originalMap : originalItem.getPinSignalMap()) {
                const ComponentPinSignalMapItem* currentMap = currentItem->getPinSignalMap().find(originalMap.getUuid()).get();
                if (!currentMap)                                                return true;
                if (currentMap->getSignalUuid() != originalMap.getSignalUuid()) return true;
            }
        }
    }
    return false;
