...
LibraryEditor::LibraryEditor(workspace::Workspace& ws, QSharedPointer<Library> lib) :
    ...
    mCurrentEditorWidget(nullptr), mLock(lib->getFilePath())
{
    ...
    connect(mUi->actionSave, &QAction::triggered,
            this, &LibraryEditor::saveTriggered);
    connect(mUi->actionRemoveElement, &QAction::triggered,
            this, &LibraryEditor::removeElementTriggered);
    connect(mUi->actionUpdateLibraryDb, &QAction::triggered,
            &mWorkspace.getLibraryDb(), &workspace::WorkspaceLibraryDb::startLibraryRescan);
    connect(mUi->actionRotateCw, &QAction::triggered,
            this, &LibraryEditor::rotateCwTriggered);
    connect(mUi->actionRotateCcw, &QAction::triggered,
            this, &LibraryEditor::rotateCcwTriggered);
    ...
    connect(mUi->actionZoomOut, &QAction::triggered,
            this, &LibraryEditor::zoomOutTriggered);
    connect(mUi->actionZoomAll, &QAction::triggered,
            this, &LibraryEditor::zoomAllTriggered);
    connect(mUi->actionGridProperties, &QAction::triggered,
            this, &LibraryEditor::editGridPropertiesTriggered);
    ...
    connect(&mWorkspace.getLibraryDb(), &workspace::WorkspaceLibraryDb::scanStarted,
            mUi->statusBar, &StatusBar::showProgressBar, Qt::QueuedConnection);
    connect(&mWorkspace.getLibraryDb(), &workspace::WorkspaceLibraryDb::scanSucceeded,
            mUi->statusBar, &StatusBar::hideProgressBar, Qt::QueuedConnection);
    connect(&mWorkspace.getLibraryDb(), &workspace::WorkspaceLibraryDb::scanProgressUpdate,
            mUi->statusBar, &StatusBar::setProgressBarPercent, Qt::QueuedConnection);

    // if the library was opened in read-only mode, we guess that it's a remote library
    // and thus show a warning that all modifications are lost after the next update
    ...
    mToolsActionGroup->addAction(EditorWidgetBase::Tool::SELECT, mUi->actionToolSelect);
    ...
    mToolsActionGroup->addAction(EditorWidgetBase::Tool::ADD_PINS, mUi->actionAddSymbolPin);
    mToolsActionGroup->addAction(EditorWidgetBase::Tool::ADD_THT_PADS, mUi->actionAddThtPad);
    mToolsActionGroup->addAction(EditorWidgetBase::Tool::ADD_SMT_PADS, mUi->actionAddSmtPad);
    mToolsActionGroup->addAction(EditorWidgetBase::Tool::ADD_HOLES, mUi->actionAddHole);
    ...
    LibraryOverviewWidget* overviewWidget = new LibraryOverviewWidget(context, lib);
    connect(overviewWidget, &LibraryOverviewWidget::windowTitleChanged,
            this, &LibraryEditor::updateTabTitles);
    connect(overviewWidget, &LibraryOverviewWidget::dirtyChanged,
            this, &LibraryEditor::updateTabTitles);
    connect(overviewWidget, &LibraryOverviewWidget::editComponentCategoryTriggered,
    ...
    connect(overviewWidget, &LibraryOverviewWidget::editSymbolTriggered,
            this, &LibraryEditor::editSymbolTriggered);
    connect(overviewWidget, &LibraryOverviewWidget::editPackageTriggered,
            this, &LibraryEditor::editPackageTriggered);
    connect(overviewWidget, &LibraryOverviewWidget::editComponentTriggered,
            this, &LibraryEditor::editComponentTriggered);
    connect(overviewWidget, &LibraryOverviewWidget::editDeviceTriggered,
    ...
    if (wizard.exec() == QDialog::Accepted) {
        ...
        switch (wizard.getContext().mElementType) {
            case NewElementWizardContext::ElementType::ComponentCategory:
                editLibraryElementTriggered<ComponentCategory, ComponentCategoryEditorWidget>(fp, true);
                break;
            case NewElementWizardContext::ElementType::PackageCategory:
                editLibraryElementTriggered<PackageCategory, PackageCategoryEditorWidget>(fp, true);
                break;
...
void LibraryEditor::removeElementTriggered() noexcept
{
    ...
    int ret = QMessageBox::warning(this, tr("Remove library element"), QString(tr(
        "WARNING: Library elements must normally NOT be removed because this will break "
        "other elements which depend on this one! They should be just marked as "
        "deprecated instead.\n\nAre you still sure to delete the whole library element "
        "\"%1\"?\n\nThis cannot be undone!")).arg(mCurrentEditorWidget->windowTitle()),
...
{
    editLibraryElementTriggered<ComponentCategory, ComponentCategoryEditorWidget>(fp, false);
}

void LibraryEditor::editPackageCategoryTriggered(const FilePath& fp) noexcept
{
    editLibraryElementTriggered<PackageCategory, PackageCategoryEditorWidget>(fp, false);
}
...
void LibraryEditor::editLibraryElementTriggered(const FilePath& fp, bool isNewElement) noexcept
{
    try {
        ...
        connect(widget, &QWidget::windowTitleChanged, this, &LibraryEditor::updateTabTitles);
        connect(widget, &EditorWidgetBase::cursorPositionChanged,
                mUi->statusBar, &StatusBar::setAbsoluteCursorPosition);
        connect(widget, &EditorWidgetBase::dirtyChanged, this, &LibraryEditor::updateTabTitles);
...
{
    setActiveEditorWidget(dynamic_cast<EditorWidgetBase*>(mUi->tabWidget->widget(index)));
}
...
void LibraryEditor::updateTabTitles() noexcept
{
    for (int i = 0; i < mUi->tabWidget->count(); ++i) {
        QWidget* widget = mUi->tabWidget->widget(i);
        const EditorWidgetBase* editorWidget = dynamic_cast<EditorWidgetBase*>(widget);
        if (editorWidget) {
