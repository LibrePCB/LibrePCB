    ...
    mAttributeProvider(attrProvider), mDrawBoundingRect(false), mLocaleOrder(localeOrder)
{
    ...
    mStrokeTexts = footprint.getStrokeTexts(); // copy texts because we modify them...
    for (StrokeText& text : mStrokeTexts) {
        text.setFont(&qApp->getDefaultStrokeFont());
        text.setAttributeProvider(this);
        StrokeTextGraphicsItem* item = new StrokeTextGraphicsItem(text, layerProvider);
        item->setParentItem(this);
    }
}
...
{
    qDeleteAll(childItems()); // remove now because childs have references to mStrokeTexts
}
