...
SymbolPreviewGraphicsItem::SymbolPreviewGraphicsItem(
  ...
  try {
    ...
    for (const SymbolPin& pin : symbol.getPins()) {
      ...
      if (mSymbVarItem)
        mapItem = mSymbVarItem->getPinSignalMap()
                      .find(pin.getUuid())
                      .get();  // can throw
      if (mapItem) displayType = mapItem->getDisplayType();
      SymbolPinPreviewGraphicsItem* item = new SymbolPinPreviewGraphicsItem(
          layerProvider, pin, signal, displayType);
...
void SymbolPreviewGraphicsItem::setDrawBoundingRect(bool enable) noexcept {
  ...
  foreach (QGraphicsItem* child, childItems()) {
    SymbolPinPreviewGraphicsItem* pin =
        dynamic_cast<SymbolPinPreviewGraphicsItem*>(child);
...
void SymbolPreviewGraphicsItem::updateCacheAndRepaint() noexcept {
  ...
  for (const Text& text : mSymbol.getTexts()) {
    ...
    if (props.rotate180) {
      props.align = 0;
      if (text.getAlign().getV() == VAlign::top())
        props.align |= Qt::AlignBottom;
      if (text.getAlign().getV() == VAlign::center())
        props.align |= Qt::AlignVCenter;
      if (text.getAlign().getV() == VAlign::bottom())
        props.align |= Qt::AlignTop;
      if (text.getAlign().getH() == HAlign::left())
        props.align |= Qt::AlignRight;
      if (text.getAlign().getH() == HAlign::center())
        props.align |= Qt::AlignHCenter;
      if (text.getAlign().getH() == HAlign::right())
        props.align |= Qt::AlignLeft;
    } else
...
QString SymbolPreviewGraphicsItem::getBuiltInAttributeValue(
