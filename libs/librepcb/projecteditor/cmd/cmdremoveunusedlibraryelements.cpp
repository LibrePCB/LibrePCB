...
CmdRemoveUnusedLibraryElements::CmdRemoveUnusedLibraryElements(Project& project) noexcept :
...
bool CmdRemoveUnusedLibraryElements::performExecute()
{
    ...
    foreach (const ComponentInstance* ci, mProject.getCircuit().getComponentInstances()) { Q_ASSERT(ci);
        usedComponents.insert(&ci->getLibComponent());
    }
    foreach (const Board* brd, mProject.getBoards()) { Q_ASSERT(brd);
        foreach (const BI_Device* dev, brd->getDeviceInstances()) { Q_ASSERT(dev);
            usedPackages.insert(&dev->getLibPackage());
            usedDevices.insert(&dev->getLibDevice());
        }
    }
    foreach (const Schematic* sch, mProject.getSchematics()) { Q_ASSERT(sch);
        foreach (const SI_Symbol* si, sch->getSymbols()) { Q_ASSERT(si);
