...
bool CmdRemoveBoardItems::performExecute()
{
    ...
    // TODO: we shouldn't do that! but currenlty it leads to errors when not doing it...
    ...
    foreach (BI_NetPoint* netpoint, mNetPoints) {
        ...
        netSegmentItemsToRemove[&netpoint->getNetSegment()].netpoints.insert(netpoint);
    }
    ...
    for (auto it = netSegmentItemsToRemove.begin(); it != netSegmentItemsToRemove.end(); ++it) {
        Q_ASSERT(it.key()->isAddedToBoard());
        bool removeAllVias = (it.value().vias == it.key()->getVias().toSet());
        bool removeAllNetLines = (it.value().netlines == it.key()->getNetLines().toSet());
        if (removeAllVias && removeAllNetLines) {
            // all items of the netsegment are selected --> remove the whole netsegment
...
void CmdRemoveBoardItems::splitUpNetSegment(BI_NetSegment& netsegment,
                                            const NetSegmentItems& itemsToRemove)
{
    // determine all resulting sub-netsegments
    QVector<NetSegmentItems> subsegments = getNonCohesiveNetSegmentSubSegments(netsegment,
                                                                               itemsToRemove);
                                                 ...
                                                 const NetSegmentItems& items)
{
    ...
    foreach (const BI_NetPoint* netpoint, items.netpoints) {
        BI_NetPoint* newNetPoint = cmdAddElements->addNetPoint(netpoint->getPosition());
    ...
    foreach (const BI_NetLine* netline, items.netlines) {
        BI_NetLineAnchor* p1 = anchorMap.value(&netline->getStartPoint(), &netline->getStartPoint()); Q_ASSERT(p1);
        BI_NetLineAnchor* p2 = anchorMap.value(&netline->getEndPoint(), &netline->getEndPoint()); Q_ASSERT(p2);
        BI_NetLine* newNetLine = cmdAddElements->addNetLine(*p1, *p2, netline->getLayer(), netline->getWidth());
        Q_ASSERT(newNetLine);
    }
        ...
        BI_NetSegment& segment, const NetSegmentItems& removedItems) noexcept
{
    ...
    while (netlines.count() + vias.count() > 0) {
        ...
        BI_NetLineAnchor* nextAnchor = netlines.count() > 0
                                       ...
                                       : vias.values().first();
        findAllConnectedNetPointsAndNetLines(*nextAnchor,
                                             processedAnchors,
        ...
        QSet<BI_NetLine*>& availableNetLines) const noexcept
{
    ...
    foreach (BI_NetLine* line, anchor.getNetLines()) {
        if (availableNetLines.contains(line) && (!netlines.contains(line))) {
            ...
            BI_NetLineAnchor* p2 = line->getOtherPoint(anchor); Q_ASSERT(p2);
            if (!processedAnchors.contains(p2)) {
                findAllConnectedNetPointsAndNetLines(*p2, processedAnchors,
