...
bool CmdCombineBoardNetSegments::performExecute()
{
    ...
    foreach (BI_NetPoint* netpoint, mOldSegment.getNetPoints()) {
        if (netpoint == &mOldAnchor) {
            ...
            BI_NetPoint* newNetPoint = cmdAdd->addNetPoint(netpoint->getPosition()); Q_ASSERT(newNetPoint);
            anchorMap.insert(netpoint, newNetPoint);
        }
    }
    foreach (BI_NetLine* netline, mOldSegment.getNetLines()) {
        BI_NetLineAnchor* startPoint = anchorMap.value(&netline->getStartPoint(), &netline->getStartPoint()); Q_ASSERT(startPoint);
        BI_NetLineAnchor* endPoint = anchorMap.value(&netline->getEndPoint(), &netline->getEndPoint()); Q_ASSERT(endPoint);
        BI_NetLine* newNetLine = cmdAdd->addNetLine(*startPoint, *endPoint, netline->getLayer(), netline->getWidth()); Q_ASSERT(newNetLine);
    }
