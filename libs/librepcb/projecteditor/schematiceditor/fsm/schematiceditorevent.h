...
public:
  ...
  enum EventType_t {
    ...
    StartAddComponent,  ///< @see
                        ///< ::librepcb::project::editor::SEE_StartAddComponent
    SwitchToSchematicPage,  ///< @see
                            ///< ::librepcb::project::editor::SEE_SwitchToSchematicPage
  };
...
public:
  ...
  static QEvent* getQEventFromSEE(const SEE_Base* see) noexcept {
    const SEE_RedirectedQEvent* r =
        dynamic_cast<const SEE_RedirectedQEvent*>(see);
 ...
 *
 * If someone (the user or the application) wants to switch to another schematic
 * page in the schematic editor, this is not allowed at any time (for example,
 * while drawing a netline in the active schematic, you cannot switch to another
 * schematic). So this type of event must be processed by the FSM. The FSM then
 * will only decide whether changing the schematic is allowed (event accepted)
