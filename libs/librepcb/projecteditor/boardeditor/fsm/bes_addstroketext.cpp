...
BES_AddStrokeText::BES_AddStrokeText(BoardEditor& editor, Ui::BoardEditor& editorUi,
...
BES_Base::ProcRetVal BES_AddStrokeText::process(BEE_Base* event) noexcept
{
    switch (event->getType()) {
        ...
        case BEE_Base::Edit_RotateCW:       return processRotateEvent(-Angle::deg90());
        case BEE_Base::Edit_RotateCCW:      return processRotateEvent(Angle::deg90());
        case BEE_Base::Edit_FlipHorizontal: return processFlipEvent(Qt::Horizontal);
        case BEE_Base::Edit_FlipVertical:   return processFlipEvent(Qt::Vertical);
...
bool BES_AddStrokeText::entry(BEE_Base* event) noexcept
{
    ...
    connect(mTextComboBox.data(), &QComboBox::currentTextChanged,
            this, &BES_AddStrokeText::textComboBoxValueChanged);
    ...
    connect(mMirrorCheckBox.data(), &QCheckBox::toggled,
            this, &BES_AddStrokeText::mirrorCheckBoxToggled);
...
BES_Base::ProcRetVal BES_AddStrokeText::processSceneEvent(BEE_Base* event) noexcept
{
...
BES_Base::ProcRetVal BES_AddStrokeText::processRotateEvent(const Angle& angle) noexcept
{
    Board* board = mEditor.getActiveBoard();
    Q_ASSERT(board); if (!board) return PassToParentState;
...
BES_Base::ProcRetVal BES_AddStrokeText::processFlipEvent(Qt::Orientation orientation) noexcept
{
    Board* board = mEditor.getActiveBoard();
    Q_ASSERT(board); if (!board) return PassToParentState;
...
void BES_AddStrokeText::layerComboBoxLayerChanged(const QString& layerName) noexcept
{
...
void BES_AddStrokeText::textComboBoxValueChanged(const QString& value) noexcept
{
