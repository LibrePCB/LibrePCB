...
BES_Base::ProcRetVal BES_DrawPlane::processSubStateIdle(
...
BES_Base::ProcRetVal BES_DrawPlane::processIdleSceneEvent(
    BEE_Base* event) noexcept {
...
BES_Base::ProcRetVal BES_DrawPlane::processSubStatePositioning(
    BEE_Base* event) noexcept {
  switch (event->getType()) {
...
BES_Base::ProcRetVal BES_DrawPlane::processPositioningSceneEvent(
    BEE_Base* event) noexcept {
  QEvent* qevent = BEE_RedirectedQEvent::getQEventFromBEE(event);
  Q_ASSERT(qevent);
  if (!qevent) return PassToParentState;
...
bool BES_DrawPlane::addSegment(Board& board, const Point& pos) noexcept {
  ...
  try {
    ...
    if (mCurrentPlane->getOutline().getVertices().count() > 2) {
      mUndoStack.appendToCmdGroup(mCmdEditCurrentPlane);
      mCmdEditCurrentPlane = nullptr;
