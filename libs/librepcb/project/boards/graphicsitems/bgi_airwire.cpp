...
void BGI_AirWire::updateCacheAndRepaint() noexcept
{
    ...
    if (mAirWire.isVertical()) {
        ...
        mLines.append(QLineF(mAirWire.getP1().toPxQPointF(),
                             mAirWire.getP2().toPxQPointF()));
        mBoundingRect = QRectF(mAirWire.getP1().toPxQPointF(),
                               mAirWire.getP2().toPxQPointF()).normalized();
    }
...
void BGI_AirWire::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget* widget)
{
    ...
    bool highlight = mAirWire.isSelected() || mAirWire.getNetSignal().isHighlighted();
