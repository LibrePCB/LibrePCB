...
UnsignedLength BI_NetLineAnchor::getMedianLineWidth() const noexcept {
    ...
    return widths.size() > 0 ? positiveToUnsigned(widths[widths.size() /2]) : UnsignedLength(0);
}
...
BI_NetLine::BI_NetLine(BI_NetSegment& segment, const BI_NetLine& other,
    ...
    BI_Base(segment.getBoard()), mNetSegment(segment), mPosition(other.mPosition),
    mUuid(Uuid::createRandom()), mStartPoint(&startPoint), mEndPoint(&endPoint),
    mLayer(nullptr), mWidth(other.mWidth)
{
    // don't just copy the pointer "mLayer" because it may come from another board!
    ...
    mWidth(node.getValueByPath<PositiveLength>("width"))
{
    ...
    mStartPoint = deserializeAnchor(node, "p1", "from", netPointAnchorMap, oldPointUuid);
    mEndPoint = deserializeAnchor(node, "p2", "to", netPointAnchorMap, oldPointUuid);
    if((!mStartPoint) || (!mEndPoint)) {
        throw RuntimeError(__FILE__, __LINE__, tr("Invalid trace anchor!"));
    }

    QString layerName = netpointLayerMap.value(oldPointUuid); // backward compatibility, remove this some time!
...
BI_NetLine::BI_NetLine(BI_NetSegment& segment,
                       BI_NetLineAnchor& startPoint,
                       BI_NetLineAnchor& endPoint,
                       GraphicsLayer& layer,
                       const PositiveLength& width) :
    BI_Base(segment.getBoard()), mNetSegment(segment), mPosition(),
    mUuid(Uuid::createRandom()), mStartPoint(&startPoint), mEndPoint(&endPoint),
    mLayer(&layer), mWidth(width)
{
    init();
}
...
BI_NetLineAnchor* BI_NetLine::getOtherPoint(const BI_NetLineAnchor& firstPoint) const noexcept
{
    ...
    Uuid& oldPointUuid) const
{
    if (const SExpression* node = root.tryGetChildByPath(oldKey)) {
        ...
        if (const SExpression* junctionNode = node->tryGetChildByPath("junction")) {
            return mNetSegment.getNetPointByUuid(junctionNode->getValueOfFirstChild<Uuid>());
...
void BI_NetLine::serializeAnchor(SExpression& root, BI_NetLineAnchor* anchor) const
{
    ...
    } else if (const BI_FootprintPad* pad = dynamic_cast<const BI_FootprintPad*>(anchor)) {
        root.appendChild("device", pad->getFootprint().getDeviceInstance().getComponentInstanceUuid(), false);
