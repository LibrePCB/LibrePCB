...
CmdFootprintStrokeTextsReset::CmdFootprintStrokeTextsReset(BI_Footprint& footprint) noexcept :
    UndoCommandGroup(tr("Reset footprint texts")),
...
bool CmdFootprintStrokeTextsReset::performExecute()
{
    ...
    // Copy all footprint texts and transform them to the global coordinate system (not
    // relative to the footprint). The original UUIDs are kept for future identification.
    for (const StrokeText& text : mFootprint.getDeviceInstance().getLibFootprint().getStrokeTexts()) {
        QScopedPointer<BI_StrokeText> newText(new BI_StrokeText(mFootprint.getBoard(), text));
        ...
        cmd.setPosition(newText->getText().getPosition() + mFootprint.getPosition(), true);
