...
AttributeListEditorWidget::AttributeListEditorWidget(QWidget* parent) noexcept
  ...
  mTable->setHorizontalHeaderItem(COLUMN_TYPE,
                                  new QTableWidgetItem(tr("Type")));
  mTable->setHorizontalHeaderItem(COLUMN_VALUE,
                                  new QTableWidgetItem(tr("Value")));
  mTable->setHorizontalHeaderItem(COLUMN_UNIT,
                                  new QTableWidgetItem(tr("Unit")));
  mTable->setHorizontalHeaderItem(COLUMN_BUTTONS,
                                  new QTableWidgetItem(tr("Actions")));
  mTable->horizontalHeader()->setSectionResizeMode(COLUMN_KEY,
                                                   QHeaderView::Stretch);
  mTable->horizontalHeader()->setSectionResizeMode(
      COLUMN_TYPE, QHeaderView::ResizeToContents);
  mTable->horizontalHeader()->setSectionResizeMode(COLUMN_VALUE,
                                                   QHeaderView::Stretch);
  mTable->horizontalHeader()->setSectionResizeMode(
      COLUMN_UNIT, QHeaderView::ResizeToContents);
  mTable->horizontalHeader()->setSectionResizeMode(
  ...
  connect(mTable, &QTableWidget::currentCellChanged, this,
          &AttributeListEditorWidget::currentCellChanged);
  connect(mTable, &QTableWidget::cellChanged, this,
          &AttributeListEditorWidget::tableCellChanged);
...
void AttributeListEditorWidget::setAttributeList(
    const AttributeList& list) noexcept {
  mAttributeList     = list;
  mSelectedAttribute = nullptr;
  updateTable();
}
...
void AttributeListEditorWidget::currentCellChanged(
...
void AttributeListEditorWidget::attributeTypeChanged(
  ...
  if (isNewAttributeRow(row)) {
    ...
    AttributeUnitComboBox* unitComboBox = dynamic_cast<AttributeUnitComboBox*>(
        mTable->cellWidget(row, COLUMN_UNIT));
    Q_ASSERT(unitComboBox);
...
void AttributeListEditorWidget::attributeUnitChanged(
...
void AttributeListEditorWidget::updateTable(
    const Attribute* selected) noexcept {
  blockSignals(true);
  ...
  setTableRowContent(newAttributeRow(), "", AttrTypeString::instance(), "",
                     nullptr);
...
void AttributeListEditorWidget::setTableRowContent(
    int row, const QString& key, const AttributeType& type,
  ...
  connect(typeComboBox, &AttributeTypeComboBox::currentItemChanged, this,
          &AttributeListEditorWidget::attributeTypeChanged);
  ...
  unitComboBox->setStyleSheet(
      "padding: 0px 3px 0px 3px;");  // reduce required space
  unitComboBox->setAttributeType(type);
  unitComboBox->setCurrentItem(unit);
  connect(unitComboBox, &AttributeUnitComboBox::currentItemChanged, this,
  ...
  connect(btnAddRemove, &QToolButton::clicked, this,
          &AttributeListEditorWidget::btnAddRemoveClicked);
  if (isExistingAttributeRow(row)) {
    ...
    connect(btnUp, &QToolButton::clicked, this,
            &AttributeListEditorWidget::btnUpClicked);
    ...
    connect(btnDown, &QToolButton::clicked, this,
            &AttributeListEditorWidget::btnDownClicked);
...
void AttributeListEditorWidget::getTableRowContent(
    int row, QString& key, const AttributeType*& type, QString& value,
    const AttributeUnit*& unit) const noexcept {
  // key
  const QTableWidgetItem* keyItem = mTable->item(row, COLUMN_KEY);
  Q_ASSERT(keyItem);
  key = keyItem->text();

  // type
  const AttributeTypeComboBox* typeComboBox =
      dynamic_cast<const AttributeTypeComboBox*>(
          mTable->cellWidget(row, COLUMN_TYPE));
  Q_ASSERT(typeComboBox);
  type = &typeComboBox->getCurrentItem();

  // value
  const QTableWidgetItem* valueItem = mTable->item(row, COLUMN_VALUE);
  Q_ASSERT(valueItem);
  value = valueItem->text();

  // unit
  const AttributeUnitComboBox* unitComboBox =
      dynamic_cast<const AttributeUnitComboBox*>(
          mTable->cellWidget(row, COLUMN_UNIT));
  Q_ASSERT(unitComboBox);
  unit = unitComboBox->getCurrentItem();
}

void AttributeListEditorWidget::addAttribute(
  ...
  try {
    ...
    mAttributeList.append(
        std::make_shared<Attribute>(attrKey, type, value, unit));  // can throw
    updateTable();
...
AttributeKey AttributeListEditorWidget::setKey(int            index,
...
void AttributeListEditorWidget::setType(int                  index,
                                        const AttributeType& type) noexcept {
  try {
    Attribute& attr = *mAttributeList[index];
    ...
    // update table
    mTable->setItem(indexToRow(index), COLUMN_VALUE,
                    new QTableWidgetItem(value));
    AttributeUnitComboBox* unitComboBox = dynamic_cast<AttributeUnitComboBox*>(
        mTable->cellWidget(indexToRow(index), COLUMN_UNIT));
    Q_ASSERT(unitComboBox);
    unitComboBox->setAttributeType(type);
    unitComboBox->setCurrentItem(unit);
...
QString AttributeListEditorWidget::setValue(int            index,
...
void AttributeListEditorWidget::setUnit(int                  index,
                                        const AttributeUnit* unit) noexcept {
  try {
    Attribute& attr = *mAttributeList[index];
...
int AttributeListEditorWidget::getRowOfTableCellWidget(QObject* obj) const
    noexcept {
  bool ok  = false;
...
AttributeKey AttributeListEditorWidget::convertStringToKeyOrThrow(
...
void AttributeListEditorWidget::throwIfValueInvalid(
    const AttributeType& type, const QString& value) const {
  if (!type.isValueValid(value)) {
    throw RuntimeError(__FILE__, __LINE__,
