...
/**
 * @brief The SerializableObjectList class implements a list of librepcb::SerializableObject
 *
 ...
 * - Undo commands librepcb::CmdListElementInsert, librepcb::CmdListElementRemove and
 *   ibrepcb::CmdListElementsSwap.
 ...
 *          into a `std::shared_ptr<T>` before adding them to the list. This is done to
 *          ensure that elements never have to be copied or moved for adding or removing
 *          them to/from the list. Otherwise it would not be possible to use this list in
 *          undo commands as references/pointers to elements would become invalid. Using
 *          pointers ensures that the objects are located at the same address over the
 *          whole lifetime. To still minimize the risk of memory leaks, `std::shared_ptr`
 *          is used instead of raw pointers.
 *
 * @warning Using Qt's `foreach` keyword on a #SerializableObjectList is not recommended
 *          because it always creates a deep copy of the list! You should use range based
 *          for loops (since C++11) instead.
 */
    ...
    public:
        ...
        class IF_Observer {
            public:
                virtual void listObjectAdded(const SerializableObjectList<T, P>& list,
                                             int newIndex, const std::shared_ptr<T>& ptr) noexcept = 0;
                virtual void listObjectRemoved(const SerializableObjectList<T, P>& list,
                                               int oldIndex, const std::shared_ptr<T>& ptr) noexcept = 0;
        };
        ...
        class Iterator {
            ...
            public:
                ...
                std::shared_ptr<O> ptr() noexcept {return std::const_pointer_cast<O>(*it);}
                ~Iterator() {}
        };
        ...
        int indexOf(const T* obj) const noexcept {
            for (int i = 0; i < count(); ++i) {if (mObjects[i].get() == obj) {return i;}}
            return -1;
        }
        int indexOf(const Uuid& key) const noexcept {
            for (int i = 0; i < count(); ++i) {if (mObjects[i]->getUuid() == key) {return i;}}
            return -1;
        }
        int indexOf(const QString& name) const noexcept {
            for (int i = 0; i < count(); ++i) {if (mObjects[i]->getName() == name) {return i;}}
            return -1;
        }
        ...
        std::shared_ptr<const T> value(int index) const noexcept {return std::const_pointer_cast<const T>(mObjects.value(index));}
        std::shared_ptr<T> find(const Uuid& key) noexcept {return value(indexOf(key));}
        std::shared_ptr<const T> find(const Uuid& key) const noexcept {return value(indexOf(key));}
        std::shared_ptr<T> find(const QString& name) noexcept {return value(indexOf(name));}
        std::shared_ptr<const T> find(const QString& name) const noexcept {return value(indexOf(name));}

        // "Hard" Element Access (assertion or exception if not found!)
        std::shared_ptr<const T> at(int index) const noexcept {return std::const_pointer_cast<const T>(mObjects.at(index));} // always read-only!
        ...
        // Observer Methods
        void registerObserver(IF_Observer* o) noexcept {Q_ASSERT(o); mObservers.append(o);}
        void unregisterObserver(IF_Observer* o) noexcept {Q_ASSERT(o); mObservers.removeOne(o);}
