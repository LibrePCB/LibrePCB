 ...
 *
 * @note This map guarantees to always contain a valid default value. A default value has
 *       an empty string as key and will be used as fallback for #value().
 *
 * @warning When modifying this class, make sure that it still guarantees to always
 *          contain a valid default value! So, don't add a default constructor, a
...
class SerializableKeyValueMap final : public SerializableObject
{
    ...
    public:
        ...
        explicit SerializableKeyValueMap(const typename T::ValueType& defaultValue) noexcept {
        ...
        explicit SerializableKeyValueMap(const SExpression& node) {
            foreach (const SExpression& child, node.getChildren(T::tagname)) {
                ...
                mValues.insert(key, deserializeFromSExpression<typename T::ValueType>(value, false)); // can throw
            }
        ...
        const typename T::ValueType& value(const QStringList& keyOrder, QString* usedKey = nullptr) const noexcept {
