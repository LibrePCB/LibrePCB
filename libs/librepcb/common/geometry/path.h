 ...
 * width or layer. It's only a list of coordinates which are connected together by
 * straight lines or circular arc segments.
 *
 * A path may be closed (first point == last point) or open (first point != last point).
 *
 * For a valid path, minimum two vertices are required. Paths with less than two vertices
 * are useless and thus considered as invalid.
 */
class Path final : public SerializableObject
{
    public:
        ...
        explicit Path(const QVector<Vertex>& vertices) noexcept : mVertices(vertices) {}
    ...
    private: // Methods
        void invalidatePainterPath() const noexcept {mPainterPathPx = QPainterPath();}
...
inline uint qHash(const Path& key, uint seed = 0) noexcept {
    ...
    foreach (const Vertex& v, key.getVertices()) {
        // from https://code.woboq.org/qt5/qtbase/src/corelib/tools/qhashfunctions.h.html
