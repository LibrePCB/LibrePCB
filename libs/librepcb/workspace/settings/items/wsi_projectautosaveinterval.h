 ...
 * interval setting
 *
 * This setting is used by the class ::librepcb::project::Project for the
 * autosave mechanism. A value of zero means that the autosave mechanism is
 * disabled! A value greater than zero defines the time interval in seconds.
 */
...
public:
  ...
  WSI_ProjectAutosaveInterval(const WSI_ProjectAutosaveInterval& other) =
      delete;
  explicit WSI_ProjectAutosaveInterval(const SExpression& node);
  ...
  QString getLabelText() const noexcept {
    return tr("Project Autosave Interval:");
  }
