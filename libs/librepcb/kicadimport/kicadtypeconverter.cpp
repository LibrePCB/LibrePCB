...
ElementName KiCadTypeConverter::convertElementName(const QString& n) {
...
QString KiCadTypeConverter::convertElementDescription(const QString& d) {
  QTextDocument doc;
...
ElementName KiCadTypeConverter::convertComponentName(QString n) {
...
ElementName KiCadTypeConverter::convertDeviceName(const QString& deviceSetName,
...
ComponentPrefix KiCadTypeConverter::convertComponentPrefix(const QString& p) {
  return ComponentPrefix(
      cleanComponentPrefix(p));  // Can theoretically throw, but should not.
}

ComponentSymbolVariantItemSuffix KiCadTypeConverter::convertGateName(
    const QString& n) {
  return ComponentSymbolVariantItemSuffix(
      cleanComponentSymbolVariantItemSuffix(n));
}

CircuitIdentifier KiCadTypeConverter::convertPinOrPadName(const QString& n) {
...
QString KiCadTypeConverter::convertInversionSyntax(const QString& s) noexcept {
  QString out;
...
/*std::shared_ptr<Attribute> KiCadTypeConverter::tryConvertAttribute(
...
/*void KiCadTypeConverter::tryConvertAttributes(
...
void KiCadTypeConverter::tryExtractMpnAndManufacturer(
...
const Layer* KiCadTypeConverter::tryConvertSchematicLayer(int id) noexcept {
  switch (id) {
    case 90:  // modules
...
const Layer* KiCadTypeConverter::tryConvertBoardLayer(int id) noexcept {
  switch (id) {
    case 1:  // tCu
...
QHash<const Layer*, const Layer*> KiCadTypeConverter::convertLayerSetup(
    const QString& s) {
  QString tmp = s;
...
/*Alignment KiCadTypeConverter::convertAlignment(parseagle::Alignment a) {
  switch (a) {
...
Length KiCadTypeConverter::convertLength(double l) {
  return Length::fromMm(l);
}

UnsignedLength KiCadTypeConverter::convertLineWidth(double w, int layerId) {
  Length l;
  switch (layerId) {
    case 20:  // dimension
    case 46:  // milling
...
/*template <>
Length KiCadTypeConverter::convertParamTo(const parseagle::Param& p) {
...
/*template <>
UnsignedLength KiCadTypeConverter::convertParamTo(const parseagle::Param& p) {
  return UnsignedLength(convertParamTo<Length>(p));
}

template <>
PositiveLength KiCadTypeConverter::convertParamTo(const parseagle::Param& p) {
  return PositiveLength(convertParamTo<Length>(p));
}

template <>
Ratio KiCadTypeConverter::convertParamTo(const parseagle::Param& p) {
  double value;
...
template <>
UnsignedRatio KiCadTypeConverter::convertParamTo(const parseagle::Param& p) {
  return UnsignedRatio(convertParamTo<Ratio>(p));
}

Point KiCadTypeConverter::convertPoint(const parseagle::Point& p) {
  return Point::fromMm(p.x, p.y);
}*/

Angle KiCadTypeConverter::convertAngle(double a) {
  return Angle::fromDeg(a);
}

/*void KiCadTypeConverter::convertGrid(const parseagle::Grid& g,
...
Vertex KiCadTypeConverter::convertVertex(const parseagle::Vertex& v) {
  return Vertex(convertPoint(v.getPosition()), convertAngle(v.getCurve()));
}

Path KiCadTypeConverter::convertVertices(const QList<parseagle::Vertex>& v,
                                         bool close) {
  Path path;
...
QList<KiCadTypeConverter::Geometry> KiCadTypeConverter::convertAndJoinWires(
...
KiCadTypeConverter::Geometry KiCadTypeConverter::convertRectangle(
...
KiCadTypeConverter::Geometry KiCadTypeConverter::convertPolygon(
...
KiCadTypeConverter::Geometry KiCadTypeConverter::convertCircle(
...
std::shared_ptr<Hole> KiCadTypeConverter::convertHole(
...
KiCadTypeConverter::Geometry KiCadTypeConverter::convertFrame(
...
QString KiCadTypeConverter::convertTextValue(const QString& v) {
  QString value = v;
...
PositiveLength KiCadTypeConverter::convertSchematicTextSize(double s) {
  return PositiveLength(Length::fromMm(s * 2.5 / 1.778));
}

std::shared_ptr<Text> KiCadTypeConverter::tryConvertSchematicText(
...
std::shared_ptr<Text> KiCadTypeConverter::tryConvertSchematicAttribute(
...
PositiveLength KiCadTypeConverter::convertBoardTextSize(int layerId,
                                                        double size) {
...
UnsignedLength KiCadTypeConverter::convertBoardTextStrokeWidth(int layerId,
                                                               double size,
                                                               int ratio) {
  if (ratio == 0) {
    ratio = 15;
...
/*std::shared_ptr<StrokeText> KiCadTypeConverter::tryConvertBoardText(
    const parseagle::Text& t) {
...
std::shared_ptr<StrokeText> KiCadTypeConverter::tryConvertBoardAttribute(
    const parseagle::Attribute& t) {
  if (auto layer = tryConvertBoardLayer(t.getLayer())) {
    const bool mirror = t.getRotation().getMirror();
    const Angle rotation = convertAngle(t.getRotation().getAngle());
...
KiCadTypeConverter::Pin KiCadTypeConverter::convertSymbolPin(
...
std::pair<std::shared_ptr<PackagePad>, std::shared_ptr<FootprintPad>>
    KiCadTypeConverter::convertThtPad(
...
std::pair<std::shared_ptr<PackagePad>, std::shared_ptr<FootprintPad>>
    KiCadTypeConverter::convertSmtPad(const parseagle::SmtPad& p) {
  Uuid uuid = Uuid::createRandom();
...
std::shared_ptr<Circle> KiCadTypeConverter::tryConvertToSchematicCircle(
    const Geometry& g) {
  if (g.circle) {
...
std::shared_ptr<Polygon> KiCadTypeConverter::tryConvertToSchematicPolygon(
    const Geometry& g) {
  if (auto layer = tryConvertSchematicLayer(g.layerId)) {
...
QVector<Path> KiCadTypeConverter::convertBoardZoneOutline(
...
QVector<std::shared_ptr<Zone>> KiCadTypeConverter::tryConvertToBoardZones(
    const Geometry& g) {
...
std::shared_ptr<Circle> KiCadTypeConverter::tryConvertToBoardCircle(
    const Geometry& g) {
  if (g.circle) {
...
std::shared_ptr<Polygon> KiCadTypeConverter::tryConvertToBoardPolygon(
    const Geometry& g) {
  if (auto layer = tryConvertBoardLayer(g.layerId)) {
    return std::make_shared<Polygon>(Uuid::createRandom(), *layer, g.lineWidth,
                                     g.filled, g.grabArea, g.path);
...
QString KiCadTypeConverter::getLayerName(int id,
