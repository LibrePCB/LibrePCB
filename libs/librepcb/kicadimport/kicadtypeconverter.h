...
public:
  ...
  /**
   * @brief LibrePCB data structure to represent an KiCad symbol pin
   */
  struct Pin {
   ...
   *
   * Removes all invalid characters from an KiCad element name and convert it
   * to the corresponding LibrePCB type. If completely invalid, "Unnamed" will
   * be returned (no error).
   *
   * @param n   KiCad element name (e.g. "R-0805")
   *
   ...
   *
   * @param d   KiCad element description (might contain HTML)
   *
   ...
   * characters.
   *
   * @param n   KiCad component name (e.g. "R-0805-")
   *
   ...
   *
   * Like #convertElementName(), but concatenating the KiCad device set name
   * with the KiCad device name.
   *
   * @param deviceSetName   KiCad device set name
   * @param deviceName      KiCad device name
   *
   ...
   *
   * @param p   KiCad device set prefix
   *
   ...
   *
   * @param s   KiCad text possibly containing inversion signs (e.g. "!RST!/EN")
   *
   ...
   *
   * @param s   KiCad layer setup string (e.g. "[2:1+((2*3)+(14*15))+16:15]")
   *
   ...
   * outline).
   *
   * @param w         KiCad line width [mm]
   * @param layerId   KiCad layer ID
   *
   ...
   *
   * @param v   KiCad text value (e.g. ">NAME")
   *
   ...
   *                          outer pad size) if it is set to 'auto' in KiCad.
   ...
   *
   * @param outline   KiCad keepout zone outline
   * @param lineWidth KiCad keepout zone line width
   *
  ...
  /**
   * @brief Get the KiCad layer name for a given layer ID
   *
   * @param id        KiCad layer ID
   * @param fallback  The string to return if the given layer is unknown
   *
   * @return KiCad layer name (fallback if unknown)
   */
