...
void LibraryEditorTab::setWatchedFiles(
  ...
  mWatchedFileNames = filenames;
}

void LibraryEditorTab::memorizeWatchedFilesHashes(
    const TransactionalDirectory& dir) noexcept {
  mWatchedFilesTimer.stop();
  mModifiedWatchedFiles.clear();
  mWatchedFileHashes.clear();

  for (const QString& name : mWatchedFileNames) {
...
void LibraryEditorTab::registerWatchedFiles() noexcept {
  mWatchedFilesTimer.stop();
