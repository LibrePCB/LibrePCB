...
LibrariesModel::LibrariesModel(Workspace& ws, Mode mode,
  ...
  : QObject(parent), mWorkspace(ws), mMode(mode) {
  ...
  for (auto& lib : mMergedLibs) {
    ...
    if (isMarkedForInstall(lib) || isMarkedForUpdate(lib)) {
      ...
      try {
        ...
        // TODO: This should be implemented without message box some say...
...
void LibrariesModel::set_row_data(std::size_t i,
  ...
  switch (data.action) {
    case ui::Action::LibraryOpen: {
      try {
        auto libEditor =
            new LibraryEditor(mWorkspace, FilePath(s2q(data.path)), false);
        libEditor->show();
      } catch (const Exception& e) {
        // TODO: This should be implemented without message box some say...
        QMessageBox::critical(nullptr, tr("Error"), e.getMsg());
...
void LibrariesModel::updateLibraries() noexcept {
