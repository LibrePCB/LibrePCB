    ...
    case ui::TabAction::EditProperties: {
      if (item && item->path.isValid()) {
        switch (item->type) {
          ...
          case ui::LibraryTreeViewItemType::Corporate: {
            emit corporateEditorRequested(mEditor, item->path, false);
            break;
          }
          default: {
...
template <>
bool LibraryTab::autoFix(const MsgNameNotTitleCase& msg) {
...
template <>
bool LibraryTab::autoFix(const MsgMissingAuthor& msg) {
  Q_UNUSED(msg);
...
void LibraryTab::refreshLibElements() noexcept {
  ...
  mCorporatesRoot = createRootItem(ui::LibraryTreeViewItemType::Corporate);
  ...
  sortItemsRecursive(mCorporatesRoot->childs);

  mCategories->clear();
  const int count =
      mCmpCatElementCount + mPkgCatElementCount + mCorporatesElementCount;
  ...
  if (!mCorporatesRoot->childs.isEmpty()) {
    mCategories->push_back(ui::LibraryTreeViewItemData{
        ui::LibraryTreeViewItemType::Corporate,  // Type
        0,  // Level
        slint::SharedString(),  // Name (set in UI)
        slint::SharedString(),  // Summary
        mCorporatesElementCount,  // Elements
        false,  // Is external
        slint::SharedString(),  // User data
    });
  }
...
void LibraryTab::loadCorporates() {
  try {
    const QSet<FilePath> elements = Toolbox::toSet(
        mDb.getAll<Corporate>(mLibrary.getDirectory().getAbsPath()).keys());
    mCorporatesElementCount = elements.count();
    for (const FilePath& fp : elements) {
      auto item = std::make_shared<TreeItem>();
      item->type = ui::LibraryTreeViewItemType::Corporate;
      item->path = fp;
      item->userData = fp.toStr();
      mDb.getTranslations<Corporate>(fp, mLocaleOrder, &item->name,
                                     &item->summary);
      item->summary = item->summary.split("\n").first().left(200);
      mCorporatesRoot->childs.push_back(item);
      mLibElementsMap.insert(fp.toStr(), item);
    }
  } catch (const Exception& e) {
    qCritical() << "Failed to load elements:" << e.getMsg();
...
void LibraryTab::setSelectedCategory(
  ...
  const QSet<ui::LibraryTreeViewItemType> types = {
      ...
      ui::LibraryTreeViewItemType::Component,
      ui::LibraryTreeViewItemType::Device,
      ui::LibraryTreeViewItemType::Corporate,
  };
  for (auto item : items) {
...
void LibraryTab::duplicateElements(
  ...
  switch (item->type) {
    ...
    case ui::LibraryTreeViewItemType::Corporate: {
      emit corporateEditorRequested(mEditor, item->path, true);
      break;
    }
    default: {
      qCritical() << "Action on unknown item type triggered.";
