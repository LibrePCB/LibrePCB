...
void LibraryDependenciesModel::set_row_data(
  ...
  if (i < mItems.size()) {
    if (auto uuid = Uuid::tryFromString(s2q(data.uuid))) {
      if (data.checked != mCheckedUuids.contains(*uuid)) {
        if (data.checked) {
        ...
        QMetaObject::invokeMethod(this, &LibraryDependenciesModel::refresh,
                                  Qt::QueuedConnection);
      }
    }
  }
...
void LibraryDependenciesModel::refresh() noexcept {
  ...
  try {
    ...
    foreach (const FilePath& libDir, libraries) {
      ...
      // Do not offer the library itself as a dependency.
      if (uuid == mLibUuid) continue;
