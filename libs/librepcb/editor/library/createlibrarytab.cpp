...
void CreateLibraryTab::trigger(ui::Action a) noexcept {
  switch (a) {
    ...
    case ui::Action::TabOk: {
      try {
        ...
        try {
          ...
          fs->write(".gitattributes",
                    FileUtils::readFile(source));  // can throw
        } catch (Exception& e) {
          qCritical() << "Could not copy the .gitattributes file:"
                      << e.getMsg();
        }
