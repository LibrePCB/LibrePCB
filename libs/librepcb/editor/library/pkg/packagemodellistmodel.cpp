  ...
  if (auto obj = mPackage->getModels().value(i)) {
    ...
    if (data.action != ui::PackageModelAction::None) {
      ...
      QMetaObject::invokeMethod(
          this, [this, obj, a = data.action]() { trigger(obj, a); },
          Qt::QueuedConnection);
...
void PackageModelListModel::trigger(std::shared_ptr<PackageModel> obj,
                                    ui::PackageModelAction a) noexcept {
  if ((!mPackage) || (!obj) || (!mPackage->getModels().contains(obj.get())))
    return;
