...
void ComponentGateListModel::setReferences(
  ...
  if (mSignals) {
    disconnect(mSignals.get(), &ComponentSignalNameListModel::modified, this,
               &ComponentGateListModel::signalsModified);
  }
  ...
  if (mList) {
    ...
    for (auto gate : mList->values()) {
      mItems.append(std::make_shared<ComponentGateEditor>(
          mWorkspace, mLayers, mCache,
          const_cast<const Component*>(mComponent.get()), mSignals, gate,
          mUndoStack));
    }
  }
...
void ComponentGateListModel::trigger(
  ...
  try {
    if (a == ui::ComponentGateAction::MoveUp) {
      const int index = mList->indexOf(obj.get());
    ...
    case ComponentSymbolVariantItemList::Event::ElementAdded:
      mItems.insert(
          ...
          std::make_shared<ComponentGateEditor>(
              mWorkspace, mLayers, mCache,
              const_cast<const Component*>(mComponent.get()), mSignals,
...
void ComponentGateListModel::signalsModified() noexcept {
