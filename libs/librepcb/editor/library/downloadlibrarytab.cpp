...
bool DownloadLibraryTab::trigger(ui::Action a) noexcept {
  if ((a == ui::Action::SectionCancel) && (mDownload)) {
    mDownload.reset();
    ...
    return true;
  } else if ((a == ui::Action::SectionCancel) && (!mDownload)) {
    emit requestClose();
    return true;
  } else if (a == ui::Action::SectionOk) {
    try {
