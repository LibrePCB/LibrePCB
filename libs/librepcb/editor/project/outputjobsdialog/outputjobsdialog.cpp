...
void OutputJobsDialog::addClicked() noexcept {
  ...
  auto addCorporateGerberExcellon =
      [&](const Uuid& corpUuid, const WorkspaceLibraryDb::OutputJobKind kind,
          const Uuid& jobUuid) {
        try {
          std::shared_ptr<const Corporate> corp =
              mLibCache.getCorporate(corpUuid, true);  // can throw
          const OutputJobList* jobs = nullptr;
          if (kind == WorkspaceLibraryDb::OutputJobKind::Pcb) {
            jobs = &corp->getPcbOutputJobs();
          } else if (kind == WorkspaceLibraryDb::OutputJobKind::Pcb) {
            jobs = &corp->getAssemblyOutputJobs();
          } else if (kind == WorkspaceLibraryDb::OutputJobKind::Pcb) {
            jobs = &corp->getUserOutputJobs();
          } else {
  ...
  try {
    corporates = mWs.getLibraryDb().getAllLatestCorporates(
  ...
  menu.addSection(tr("Production Data"));
  {
    ...
    if (corporates) {
      ...
      for (const auto& corp : *corporates) {
        for (const auto& job : corp.outputJobs) {
          if (job.type == GerberExcellonOutputJob::getTypeName()) {
            if (!separatorAdded) {
              sub->addSeparator();
              separatorAdded = true;
            }
            sub->addAction(corp.logo, job.name, [&]() {
              addCorporateGerberExcellon(corp.uuid, job.kind, job.uuid);
            });
          }
        }
