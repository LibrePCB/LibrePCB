...
SchematicTab::SchematicTab(GuiApplication& app,
  ...
  connect(mEditor.get(), &ProjectEditor::ercFinished, this,
          &SchematicTab::baseUiDataChanged);
...
ui::SchematicTabData SchematicTab::getUiData() const noexcept {
  ...
  return ui::SchematicTabData{
      ...
      *mToolHeight % 2 == 0,  // Tool height can decrease
      ...
      q2s(toSingleLine(mToolValue)),  // Tool value
      mToolValueSuggestions,  // Tool value suggestions
      mToolAttributeValue ? q2s(toSingleLine(*mToolAttributeValue))
                          : slint::SharedString(),  // Tool attribute value
      q2s(mToolAttributeValuePlaceholder),  // Tool attribute value placeholder
      mToolAttributeValue.has_value(),  // Tool attribute value enabled
      mToolAttributeUnits,  // Tool attribute units
      static_cast<int>(mToolAttributeUnitsQt.indexOf(
          mToolAttributeUnit)),  // Tool attribute unit index
...
void SchematicTab::setUiData(const ui::SchematicTabData& data) noexcept {
  ...
  emit valueRequested(toMultiLine(s2q(data.tool_value)));
  // Unit must be set before value, because value may override the unit!
  emit attributeUnitRequested(
      mToolAttributeUnitsQt.value(data.tool_attribute_unit_index));
  emit attributeValueRequested(toMultiLine(s2q(data.tool_attribute_value)));
...
bool SchematicTab::actionTriggered(ui::ActionId id) noexcept {
  if (id == ui::ActionId::Save) {
    ...
    return true;
  } else if (id == ui::ActionId::Cut) {
    return mFsm->processCut();
  } else if (id == ui::ActionId::Copy) {
    return mFsm->processCopy();
  } else if (id == ui::ActionId::Paste) {
    return mFsm->processPaste();
  } else if (id == ui::ActionId::Delete) {
    return mFsm->processRemove();
  } else if (id == ui::ActionId::RotateCcw) {
    return mFsm->processRotate(Angle::deg90());
  } else if (id == ui::ActionId::RotateCw) {
    return mFsm->processRotate(-Angle::deg90());
  } else if (id == ui::ActionId::MirrorHorizontally) {
    return mFsm->processMirror(Qt::Horizontal);
  } else if (id == ui::ActionId::MirrorVertically) {
    return mFsm->processMirror(Qt::Vertical);
    //} else if (id == ui::ActionId::MoveAlign) {
    //  return mFsm->process();
    //} else if (id == ui::ActionId::SnapToGrid) {
    //  return mFsm->process();
  } else if (id == ui::ActionId::ResetTexts) {
    return mFsm->processResetAllTexts();
    //} else if (id == ui::ActionId::LockUnlockPlacement) {
    //  return mFsm->process();
  } else if (id == ui::ActionId::EditProperties) {
...
void SchematicTab::fsmToolEnter(
  ...
  // Layers
  mToolLayersQt = Toolbox::sortedQSet(state.getLayers());
  mToolLayers->clear();
...
void SchematicTab::fsmToolEnter(SchematicEditorState_AddText& state) noexcept {
  ...
  mFsmStateConnections.append(
      connect(this, &SchematicTab::heightRequested, &state,
