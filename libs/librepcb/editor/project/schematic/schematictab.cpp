...
SchematicTab::SchematicTab(GuiApplication& app, SchematicEditor& editor,
  ...
  connect(&mProjectEditor, &ProjectEditor::ercFinished, this,
          [this]() { onUiDataChanged.notify(); });
...
ui::TabData SchematicTab::getUiData() const noexcept {
  ...
  std::shared_ptr<RuleCheckMessagesModel> ercMessages =
      mProjectEditor.getErcMessages();
  const ui::RuleCheckState ercState = ercMessages
  ...
  return ui::TabData{
      ...
      q2s(mProjectEditor.getErcExecutionError()),  // Rule check execution error
  };
}
