      ...
      if (devItem) {
        ...
        if ((!mSelectedDevice) ||
          ...
          std::shared_ptr<Device> device(
              Device::open(std::make_unique<TransactionalDirectory>(
...
void AddComponentDialog::searchComponents(
  ...
  if (input.length() > 1) {
    ...
    for (auto cmpIt = result.components.begin();
      ...
      cmpItem->setIcon(0, EditorToolbox::svgIcon(":/bi/cpu.svg"));
...
void AddComponentDialog::setSelectedDevice(std::shared_ptr<const Device> dev) {
  ...
  if (mSelectedDevice) {
    ...
    if (pkgFp.isValid()) {
      mSelectedPackage = Package::open(std::make_unique<TransactionalDirectory>(
