...
Board3dTab::Board3dTab(GuiApplication& app, std::shared_ptr<ProjectEditor2> prj,
                       int boardIndex, QObject* parent) noexcept
  : WindowTab(app, parent),
    onDerivedUiDataChanged(*this),
    mEditor(prj),
    mObjIndex(boardIndex),
    mFrameIndex(0),
...
ui::Board3dTabData Board3dTab::getDerivedUiData() const noexcept {
  return ui::Board3dTabData{
      ...
      q2s(Qt::white),  // Overlay text color
...
void Board3dTab::activate() noexcept {
  if (auto brd = mEditor->getProject().getBoardByIndex(mObjIndex)) {
    ...
    auto av = mEditor->getProject().getCircuit().getAssemblyVariants().value(0);
...
void Board3dTab::triggerAsync(ui::Action a) noexcept {
  if (a == ui::Action::Save) {
    mEditor->saveProject();
    return;
  } else if (a == ui::Action::ZoomIn) {
    zoom(QPointF(mViewSize.width() / 2, mViewSize.height() / 2), 1.3);
    return;
  } else if (a == ui::Action::ZoomOut) {
    zoom(QPointF(mViewSize.width() / 2, mViewSize.height() / 2), 1 / 1.3);
    return;
  } else if (a == ui::Action::ZoomFit) {
