...
Board2dTab::Board2dTab(GuiApplication& app, BoardEditor& editor,
  ...
  // Connect DRC.
  connect(mDrc.get(), &BoardDesignRuleCheck::started, this,
          [this]() { emit panelPageRequested(ui::PanelPage::RuleCheck); });
...
void Board2dTab::trigger(ui::TabAction a) noexcept {
  ...
  if (a == ui::TabAction::Print) {
    ...
    return;
  } else if (a == ui::TabAction::Cut) {
    mFsm->processCut();
    return;
  } else if (a == ui::TabAction::Copy) {
    mFsm->processCopy();
    return;
  } else if (a == ui::TabAction::Paste) {
    mFsm->processPaste();
    return;
  } else if (a == ui::TabAction::Delete) {
    mFsm->processRemove();
    return;
  } else if (a == ui::TabAction::RotateCcw) {
    mFsm->processRotate(Angle::deg90());
    return;
  } else if (a == ui::TabAction::RotateCw) {
    mFsm->processRotate(-Angle::deg90());
    return;
  } else if (a == ui::TabAction::FlipHorizontally) {
    mFsm->processFlip(Qt::Horizontal);
    return;
  } else if (a == ui::TabAction::FlipVertically) {
    mFsm->processFlip(Qt::Vertical);
    return;
  } else if (a == ui::TabAction::MoveLeft) {
    if (!mFsm->processMove(Point(-mBoard.getGridInterval(), 0))) {
      mView->scrollLeft();
    }
    return;
  } else if (a == ui::TabAction::MoveRight) {
    if (!mFsm->processMove(Point(*mBoard.getGridInterval(), 0))) {
      mView->scrollRight();
    }
    return;
  } else if (a == ui::TabAction::MoveUp) {
    if (!mFsm->processMove(Point(0, *mBoard.getGridInterval()))) {
      mView->scrollUp();
    }
    return;
  } else if (a == ui::TabAction::MoveDown) {
    if (!mFsm->processMove(Point(0, -mBoard.getGridInterval()))) {
      mView->scrollDown();
    }
    return;
  } else if (a == ui::TabAction::SnapToGrid) {
    mFsm->processSnapToGrid();
    return;
  } else if (a == ui::TabAction::Lock) {
    mFsm->processSetLocked(true);
    return;
  } else if (a == ui::TabAction::Unlock) {
    mFsm->processSetLocked(false);
    return;
  } else if (a == ui::TabAction::LineWidthIncrease) {
    mFsm->processChangeLineWidth(1);
    return;
  } else if (a == ui::TabAction::LineWidthDecrease) {
    mFsm->processChangeLineWidth(-1);
    return;
  } else if (a == ui::TabAction::LineWidthSet) {
    mFsm->processChangeLineWidth(0);
    return;
  } else if (a == ui::TabAction::ResetTexts) {
    mFsm->processResetAllTexts();
    return;
  } else if (a == ui::TabAction::EditProperties) {
    mFsm->processEditProperties();
    return;
  } else if (a == ui::TabAction::GridIntervalIncrease) {
    ...
    return;
  } else if (a == ui::TabAction::GridIntervalDecrease) {
    ...
    return;
  } else if (a == ui::TabAction::LayersTop) {
    mLayers->showTop();
    return;
    ...
    mLayers->showAll();
    return;
  } else if (a == ui::TabAction::LayersNone) {
    mLayers->showNone();
    return;
  } else if (a == ui::TabAction::ZoomIn) {
    mView->zoomIn();
    return;
  } else if (a == ui::TabAction::ZoomOut) {
    mView->zoomOut();
    return;
  } else if ((a == ui::TabAction::ZoomFit) && mScene) {
    mView->zoomToSceneRect(mScene->itemsBoundingRect());
    return;
  } else if (a == ui::TabAction::FindNext) {
    mSearchContext.findNext();
    return;
  } else if (a == ui::TabAction::FindPrevious) {
    mSearchContext.findPrevious();
    return;
  } else if (a == ui::TabAction::BoardPlaceComponent) {
    addUnplacedComponentsToBoard(PlaceComponentsMode::Single);
    return;
  } else if (a == ui::TabAction::BoardPlaceComponentsSimilar) {
    addUnplacedComponentsToBoard(PlaceComponentsMode::Similar);
    return;
  } else if (a == ui::TabAction::BoardPlaceComponentsAll) {
    addUnplacedComponentsToBoard(PlaceComponentsMode::All);
    return;
  } else if (a == ui::TabAction::ToolSelect) {
    mFsm->processSelect();
    return;
  } else if (a == ui::TabAction::ToolWire) {
    mFsm->processDrawTrace();
    return;
  } else if (a == ui::TabAction::ToolVia) {
    mFsm->processAddVia();
    return;
  } else if (a == ui::TabAction::ToolPolygon) {
    mFsm->processDrawPolygon();
    return;
  } else if (a == ui::TabAction::ToolText) {
    mFsm->processAddStrokeText();
    return;
  } else if (a == ui::TabAction::ToolPlane) {
    mFsm->processDrawPlane();
    return;
  } else if (a == ui::TabAction::ToolZone) {
    mFsm->processDrawZone();
    return;
  } else if (a == ui::TabAction::ToolHole) {
    mFsm->processAddHole();
    return;
  } else if (a == ui::TabAction::ToolMeasure) {
    mFsm->processMeasure();
    return;
  }
...
void Board2dTab::setDrcResult(
  ...
  if (!mDrcMessages) {
    ...
    connect(mDrcMessages.get(), &RuleCheckMessagesModel::highlightRequested,
            this, &Board2dTab::highlightDrcMessage);
  }
  ...
  clearDrcMarker();  // Previous marker may be obsolete now.
  onUiDataChanged.notify();
}
...
void Board2dTab::highlightDrcMessage(
  ...
  if ((!msg) || msg->getLocations().isEmpty()) {
    ...
    mScene->addItem(*mDrcLocationGraphicsItem.data());
