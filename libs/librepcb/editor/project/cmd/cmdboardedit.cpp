...
CmdBoardEdit::CmdBoardEdit(Board& board) noexcept
  : UndoCommand(tr("Modify Board Setup")),
    ...
    mNewName(mOldName),
    mOldTags(mBoard.getPreferredTags()),
    mNewTags(mOldTags),
...
void CmdBoardEdit::setPreferredTas(
    const QVector<TagConditional>& tags) noexcept {
  Q_ASSERT(!wasEverExecuted());
  mNewTags = tags;
}
...
void CmdBoardEdit::performUndo() {
  ...
  mBoard.setPreferredTags(mOldTags);
...
void CmdBoardEdit::performRedo() {
  ...
  mBoard.setPreferredTags(mNewTags);
