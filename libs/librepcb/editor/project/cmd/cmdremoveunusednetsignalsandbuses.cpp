...
CmdRemoveUnusedNetSignalsAndBuses::CmdRemoveUnusedNetSignalsAndBuses(
    Circuit& circuit) noexcept
  : UndoCommandGroup(tr("Remove Unused Nets & Buses")), mCircuit(circuit) {
}
...
bool CmdRemoveUnusedNetSignalsAndBuses::performExecute() {
  ...
  foreach (Bus* bus, mCircuit.getBuses()) {
    if (!bus->isUsed()) {
      appendChild(new CmdBusRemove(mCircuit, *bus));
    }
  }
