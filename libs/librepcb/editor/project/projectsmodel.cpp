...
int ProjectsModel::getIndexOf(std::shared_ptr<ProjectEditor2> prj) noexcept {
  return mEditors.indexOf(prj);
}

std::shared_ptr<ProjectEditor2> ProjectsModel::getProject(int index) noexcept {
  return mEditors.value(index);
}

std::shared_ptr<ProjectEditor2> ProjectsModel::getProject(
    const FilePath& fp) noexcept {
  for (auto p : mEditors) {
    if (p->getProject().getPath() == fp) {
      return p;
    }
  }
  return nullptr;
...
std::shared_ptr<ProjectEditor2> ProjectsModel::openProject(
  ...
  try {
    ...
    // Open editor.
    auto editor = std::make_shared<ProjectEditor2>(mApp, std::move(project),
...
void ProjectsModel::closeProject(const FilePath& fp) noexcept {
  for (int i = 0; i < mEditors.count(); ++i) {
    if (mEditors.at(i)->getProject().getPath() == fp) {
      closeProject(i);
    }
  }
}
