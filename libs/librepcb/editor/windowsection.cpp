...
WindowSection::WindowSection(GuiApplication& app, QObject* parent) noexcept
  ...
  connect(mTabsModel.get(), &WindowTabsModel::closeTabRequested, this,
          &WindowSection::closeTab, Qt::QueuedConnection);
...
std::shared_ptr<ProjectEditor2>
    WindowSection::getCurrentProjectEditor() noexcept {
  if (std::shared_ptr<WindowTab> t = getTab(mUiData.current_tab_index)) {
    return t->getProjectEditor();
  }
  return nullptr;
}
...
void WindowSection::setCurrentTab(int index) noexcept {
  ...
  emit uiDataChanged();
  emit currentProjectChanged(getCurrentProjectEditor());
}

bool WindowSection::trigger(ui::Action a) noexcept {
  if (std::shared_ptr<WindowTab> t = getCurrentTab()) {
    if (t->trigger(a)) {
      return true;
    } else if (std::shared_ptr<ProjectEditor2> editor = t->getProjectEditor()) {
      if (a == ui::Action::Undo) {
        editor->getUndoStack().undo();  // TODO: Exception handling!
        return true;
      } else if (a == ui::Action::Redo) {
