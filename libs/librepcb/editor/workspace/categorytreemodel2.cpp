...
CategoryTreeModel2::CategoryTreeModel2(const WorkspaceLibraryDb& db,
  ...
  : QObject(parent),
    ...
    mSettings(ws),
    mFilters(filters),
    mIcon(q2s(QIcon(":/img/places/folder.png").pixmap(32, QIcon::Disabled))) {
  connect(&mDb, &WorkspaceLibraryDb::scanFinished, this,
          &CategoryTreeModel2::refresh, Qt::QueuedConnection);
  connect(&mSettings.libraryLocaleOrder, &WorkspaceSettingsItem::edited, this,
          &CategoryTreeModel2::refresh, Qt::QueuedConnection);
  refresh();
}

CategoryTreeModel2::~CategoryTreeModel2() noexcept {
}
...
std::size_t CategoryTreeModel2::row_count() const {
  return mItems.size();
}

std::optional<ui::TreeViewItemData> CategoryTreeModel2::row_data(
    std::size_t i) const {
...
void CategoryTreeModel2::refresh() noexcept {
  try {
    mItems.clear();
...
template <typename T>
void CategoryTreeModel2::loadChilds(const std::optional<Uuid>& parent,
                                    int level) {
