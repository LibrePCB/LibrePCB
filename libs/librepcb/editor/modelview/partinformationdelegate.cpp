...
bool PartInformationDelegate::Data::getColors(QBrush& background, QPen& outline,
  ...
  if (info && (info->data.results == 1)) {
    const QString sts = info->data.status.toLower();
    const std::optional<int> av = info->data.availability;
    if (sts == "preview") {
...
bool PartInformationDelegate::getData(const QModelIndex& index,
  ...
  if (d.canConvert<Data>()) {
    data = d.value<Data>();
    return (data.info && (data.info->data.results == 1)) ||
