...
void MainWindow::triggerLibraryElement(slint::SharedString path,
  ...
  switch (a) {
    ...
    case ui::LibraryElementAction::ImportEagleLibrary: {
      if (auto pair = getLibraryEditor()) {
        EagleLibraryImportWizard wiz(mApp.getWorkspace(),
                                     pair->first->getFilePath(),
                                     qApp->activeWindow());
        wiz.exec();
      }
      break;
    ...
    case ui::LibraryElementAction::ImportKicadLibrary: {
      if (auto pair = getLibraryEditor()) {
        KiCadLibraryImportWizard wiz(mApp.getWorkspace(),
                                     pair->first->getFilePath(),
                                     qApp->activeWindow());
        wiz.exec();
      }
...
void MainWindow::openComponentTab(LibraryEditor2& editor,
  ...
  if (!switchToLibraryElementTab<ComponentTab>(fp)) {
    try {
      ...
      addTab(std::make_shared<ComponentTab>(editor, std::move(cmp), false));
    } catch (const Exception& e) {
      QMessageBox::critical(mWidget, tr("Error"), e.getMsg());
    }
  }
...
void MainWindow::openDeviceTab(LibraryEditor2& editor,
  ...
  if (!switchToLibraryElementTab<DeviceTab>(fp)) {
    try {
      ...
      addTab(std::make_shared<DeviceTab>(editor, std::move(dev), false));
    } catch (const Exception& e) {
      QMessageBox::critical(mWidget, tr("Error"), e.getMsg());
    }
  }
